...
class ShippingController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->editColumn('mileage_fee', function (Shipping $data) {
                $sign = Currency::where('is_default', '=', 1)->first();
                $price = $sign->sign . $data->mileage_fee;
                return $price;
            })
            ->editColumn('trigger_miles', function (Shipping $data) {
                $sign = Currency::where('is_default', '=', 1)->first();
                $price = $data->trigger_miles;
                return $price;
            })
            ->editColumn('triggerprice', function (Shipping $data) {
                if($data->triggerprice) {
                    $sign = Currency::where('is_default', '=', 1)->first();
                    $triggerprice = $sign->sign . $data->triggerprice;
                    return $triggerprice;
                } else {
                    return '';
                }
