...
class OrderController extends Controller
{
    ...
    public function datatables($status)
    {
        ...
        } elseif ($status == 'needhelp') {
            $datas = Order::where('status', '=', 'needhelp')->get();
        } else {
    ...
    {
        return view('admin.order.needhelp');
    }
    ...
    {
        $order = Order::findOrFail($id);
        $order->pay_amount = $request->toprice;
        $order->description = $request->description;
        $order->update();
        $msg = 'Successfully Changed the Total Price';
        return response()->json($msg);
    }
}