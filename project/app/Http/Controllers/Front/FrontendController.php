...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
    ...
    public function index(Request $request)
    {
        ...
        if($solo_mode == 1) {
            ...
            ->where('category_id', $solo_category)->paginate(24);
...
// -------------------------------- STRAIN SECTION ----------------------------------------
    public function strain(Request $request) {
        return view('front.strain');
    }

    public function strain_datatables()
    {
        $datas = Strain::orderBy('name', 'asc')->get();
        ...
        return Datatables::of($datas)
            ->editColumn('name', function (Strain $data) {
                ...
                if($thumb['path'] == '') {
                    ...
                    return '<img src="'. asset($thumb['path']).'" width="25px" height="25px;" /> '.$data->name;
                }
            ...
            ->addColumn('action', function (Strain $data) {
                $edit_btn = '';
                $edit_btn = '<a class="strain-action" href="'.route('front.strainshow',$data->id).'"> Quick View</a>';
                
                return '<div style="display: inline-flex;">'.$edit_btn.'</div>';
            })
            ->rawColumns(['name', 'thumb', 'effect', 'description', 'parent', 'action'])
            ->toJson(); //--- Returning Json Data To Client Side
    }

    public function strain_inventory($id) {
...
// -------------------------------- FAQ SECTION ----------------------------------------
    public function faq()
    {
        ...
        return view('front.faq', compact('faqs', 'check21'));
    }
// -------------------------------- FAQ SECTION ENDS----------------------------------------
...
// -------------------------------- PAGE SECTION ----------------------------------------
    public function page($slug)
    {
        ...
        return view('front.page', compact('page', 'check21'));
    }
// -------------------------------- PAGE SECTION ENDS----------------------------------------
...
// -------------------------------- CONTACT SECTION ----------------------------------------
    public function contact()
    {
        ...
        $check21 = 1;
        return view('front.contact', compact('ps', 'check21'));
    }
    ...
    public function aboutus()
    {
        ...
        return view('front.aboutus', compact('ps', 'check21'));
    }
...
// -------------------------------- PRINT SECTION ENDS ----------------------------------------
    ...
    public function terms_condition() {
        ...
        $check21 = 1;
        return view('front.terms_condition', compact('check21'));
    }

    public function calc_distance(Request $request) 
    {
        $origin_lat = $request->origin_lat;
        $origin_lng = $request->origin_lng;
        $dict_lat = $request->dict_lat;
        $dict_lng = $request->dict_lng;

        $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$origin_lat.",".$origin_lng.
        "&destinations=".$dict_lat.",".$dict_lng."&key=AIzaSyDDkSnYdnbcW72L71z5h6o4yKmwyacOsvY";
        $ch = curl_init($url);
        $headers = array(
        ...
        curl_close($ch);
        $output = rtrim($output, '1');

        echo $output;
        return;
    }

    public function tawkhooks(Request $request) {
        // $signature = "jdfkajdsflS";
        // $body = json_encode($request->all());

        $data = [
            'to' => "kosong0926@hotmail.com",
            'subject' => "New Tawk Message",
            'body' => "Webhooks",
        ];
    ...
    private function verifySignature ($body, $signature) {
        ...
        $digest = hash_hmac('sha1', $body, "e7456a0eb0d9033213a4eca4764433c5aed2b591");
