...
class Product extends Model
{
    ...
    protected $fillable = ['user_id', 'category_id', 'product_type', 'affiliate_link', 'sku', 'subcategory_id', 'childcategory_id', 'location_id', 'attributes', 'name', 'photo', 'size', 'size_qty', 'size_price', 'color', 'details', 'price', 'previous_price', 'stock', 'policy', 'status', 'views', 'tags', 'featured', 'best', 'top', 'hot', 'latest', 'big', 'trending', 'sale', 'features', 'colors', 'product_condition', 'ship', 'meta_tag', 'meta_description', 'youtube', 'type', 'file', 'license', 'license_qty', 'link', 'platform', 'region', 'licence_type', 'measure', 'discount_date', 'is_discount', 'whole_sell_qty', 'whole_sell_discount', 'catalog_id', 'slug', 'effects', 'parent', 'description', 'effects', 'is_verified'];
    ...
    public function setCurrency()
    {
        ...
        if($curr->name == "Bitcoin")
            $price = round($price * $curr->value, 6);
        else
    ...
    public function showPrice()
    {
        ...
        if ($curr->name == 'Bitcoin')
            $price = number_format(($price * $curr->value), '6', '.', '');
        else
    ...
    {
        $details = $this->effects;
        return $details;
    }    
    
    public function showCategoryID()
    {
        $details = $this->category_id;
        return $details;
    } 
