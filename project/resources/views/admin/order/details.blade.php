...
@section('content')
                        ...
                        <div class="table-responsive-sm">
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <td width="10%">:</td>
                                    <td width="45%">
                                    {{$order->currency_sign}}<span id="pay_amount">{{ round($order->pay_amount * $order->currency_value , 2) }}</span>
                                    @if($order->status == 'needhelp')<a href="#" data-toggle="modal" data-target="#change-price"> <span class="fas fa-edit"></span></a>@endif
                                    </td>
                                </tr>
                                <tr>
                                ...
                                @foreach($cart->items as $key => $product)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <p>
                                                <strong>{{ __('Price') }}
                                                    :</strong> {{$order->currency_sign}}{{ round($product['item']['price'] * $order->currency_value , 2) }}
                                            </p>
                                            <p>
                                                <strong>{{ __('Qty') }}
    ...
    <div class="modal fade" id="change-price" tabindex="-1" role="dialog" aria-labelledby="modal1"
         aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="submit-loader">
                ...
                <div class="modal-header d-block text-center">
                    <h4 class="modal-title d-inline-block">{{ __('Edit Total Price') }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="edit-price" method="POST" action="{{route('admin-order-editprice', $order->id)}}">

                <!-- Modal body -->
                <div class="modal-body">
                        {{csrf_field()}}
                        <label>Total Cost (USD)</label>
                        <input type="number" step="0.01" class="input-field eml-val" id="toprice" name="toprice" placeholder="Total Price *" value="{{$order->pay_amount}}" required="">
                        <br>
                        <label>Total Cost Change Description</label>
                        <textarea class="input-field textarea" name="description" id="description" placeholder="Your Description *" required="">{{$order->description}}</textarea>
                ...
                <div class="modal-footer justify-content-center">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('Cancel') }}</button>
                    <button class="btn btn-success btn-ok order-btn" type="submit" id="edit-price-btn">{{ __('Change') }}</button>
                </div>
                </form>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('submit', '#edit-price', function (e) {
            e.preventDefault();
            $("#change-price").modal('hide');
            $('button#edit-price-btn').prop('disabled', true);
            $.ajax({
                method: "POST",
                url: $(this).prop('action'),
                data: new FormData(this),
                ...
                processData: false,
                success: function (data) {
                    $('button#edit-price-btn').prop('disabled', false);
                    if ((data.errors)) {
                        for (var error in data.errors) {
                            $.notify('<li>' + data.errors[error] + '</li>', 'error');
                        }
                        ...
                        $.notify(data, 'success');
                        $("#pay_amount").text($('[name=toprice]').val());
                    }
                }
            });
        });
